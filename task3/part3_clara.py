...
def clara_objective(n_clusters, n_sampling):
    k = int(round(n_clusters))
    n_sampling = int(round(n_sampling))
    ...
    try:
        model = CLARA(
            n_clusters=k,
            n_sampling=min(n_sampling, len(X)),
            n_sampling_iter=5,
            random_state=42
        )
        ...
        # Composite unsupervised score
        score = sil + 0.01 * np.log1p(ch) - 0.1 * db
